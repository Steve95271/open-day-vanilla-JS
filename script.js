    ...
    return programs
        ...
        <div class="program">
            <section class="program-title-type">
                <div class="program-type" style="background: ${program.programType.type_colour}">
            ...
            <section>
                ...
                <div class="location">
                    <strong>Location: </strong>${program.room}, ${program.location.title},  ${program.location.address}, ${program.location.postcode}
                </div>
                <div class="date">
                    <strong>Date:</strong> ${formatDate(program.start_time)}
                </div>
                <div class="time">
                    <strong>Time:</strong> ${formatTime(program.start_time)} - ${formatTime(program.end_time)}
                </div>
            </section>
...
function renderTopic(topics) {
    ...
    const topicHtml = topicArray.map((topic) => `
        <section class="topic">
...
function renderMainHeader(data) {
    ...
    const htmlElement = `
       ...
       <h2>
            🕰️ <i>${formatTime(data.start_time)}</i> - <i>${formatTime(data.end_time)}</i>
       </h2>
    `
    document.querySelector(".main-header").insertAdjacentHTML("beforeend", htmlElement);
}

function addTopicsNames() {
